...
class QLearning {
    ...
    constructor({ alpha, gamma, epsilon, epsilonDecaryRate }: { alpha: number, gamma: number, epsilon: number, epsilonDecaryRate: number }) {
        this.qTable = new Map();
        this.alpha = alpha;
        this.gamma = gamma;
    ...
    public chooseAction(x: number, z: number): Action {
        ...
        if (Math.random() < this.decayEpsilon()) {
            ...
            if (!qValues) return this.actions[Math.floor(Math.random() * this.actions.length)];
            return Array.from(qValues.entries()).reduce((a, b) => a[1] > b[1] ? a : b)[0];
        }
    }

    public updateQTable(x: number, z: number, action: Action, reward: number, newX: number, newZ: number): void {
        const state = this.getStateKey(x, z);
        const newState = this.getStateKey(newX, newZ);
        const oldQValue = this.getQValue(state, action);
        const maxFutureQValue = Math.max(...Array.from(this.qTable.get(newState)?.values() || [0]));
        const newQValue = oldQValue + this.alpha * (reward + this.gamma * maxFutureQValue - oldQValue);
